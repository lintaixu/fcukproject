...
class ModelEngine:
    ...
    def train_autoencoder(self, train_df, epochs=50, batch_size=32,
        ...
        n_up   = len(X_up_scaled)
        n_down = len(X_down_scaled)
        
        # ── 訓練迴圈 ──────────────────────────────────────────────────
        for epoch in range(epochs):
            # ── AE-up：以上漲樣本為主，對比下漲樣本 ──────────────────
            perm = np.random.permutation(n_up)
            epoch_loss_up = []
            for start in range(0, n_up, batch_size):
                idx = perm[start:start + batch_size]
                x_same = tf.constant(X_up_scaled[idx], dtype=tf.float32)
                # 從異類隨機取相同批次大小（允許重複抽樣）
                idx_opp = np.random.choice(n_down, size=len(idx), replace=True)
            ...
            perm = np.random.permutation(n_down)
            epoch_loss_down = []
            for start in range(0, n_down, batch_size):
                idx = perm[start:start + batch_size]
                x_same = tf.constant(X_down_scaled[idx], dtype=tf.float32)
                idx_opp = np.random.choice(n_up, size=len(idx), replace=True)
        ...
        self.history_up   = _HistoryWrapper(history_up_loss)
        self.history_down = _HistoryWrapper(history_down_loss)
        
        print(f"✓ 上漲 AE (含對比損失) 訓練完成 (最終損失: {history_up_loss[-1]:.6f})")
        print(f"✓ 下跌 AE (含對比損失) 訓練完成 (最終損失: {history_down_loss[-1]:.6f})")
    ...
    def _run_analysis_thread(self):
        ...
        try:
            ...
            if self.model_engine.history_up is not None:
                ...
                self.update_status(f"✅ 上漲型態 AE 最終損失（含對比項）: {final_loss_up:.6f}")
            
            if self.model_engine.history_down is not None:
                final_loss_down = self.model_engine.history_down.history['loss'][-1]
                self.update_status(f"✅ 下跌型態 AE 最終損失（含對比項）: {final_loss_down:.6f}")
