...
from config import (
    DATA_CONFIG, PATTERN_CONFIG, BACKTEST_CONFIG, 
    ANALYSIS_CONFIG, STOCK_SYMBOLS, ANALYSIS_PERIOD
)
...
def main():
    ...
    for symbol in symbols_to_test:
        ...
        try:
            # 1. 獲取數據
            logger.info(f"獲取歷史數據 ({ANALYSIS_PERIOD['start_date']} 到 {ANALYSIS_PERIOD['end_date']})")
            ...
            for pattern_name, indices in patterns.items():
                if len(indices) >= 5:  # 至少有5個訊號
                    ...
                    logger.info(f"  {pattern_name}: {len(trades)} 筆, "
    ...
    if combined_metrics:
        ...
        generate_html_report(combined_analysis, "台灣股市樣本 (2022-2025)", 'analysis_report.html')
