...
class PatternAnalyzer:
    ...
    def analyze_pattern_performance(self, pattern_metrics: Dict) -> Dict:
        ...
        for pattern, metrics in pattern_metrics.items():
            if metrics["total_trades"] < 10:
                ...
                sharpe_score = min(
                    max(metrics["sharpe_ratio"] / 2, 0) * 20, 20
                )  # 夏普比20分
    ...
    def generate_summary_report(self, pattern_metrics: Dict, symbol: str) -> str:
        """生成文本報告"""
...
"""
        ...
        valid_patterns = len(
            [m for m in pattern_metrics.values() if m.get("total_trades", 0) >= 10]
        )
...
"""
    ...
    def export_json(self, analysis: Dict, filename: str = "backtest_results.json"):
        ...
        with open(filename, "w", encoding="utf-8") as f:
            json.dump(
                output, f, indent=2, ensure_ascii=False, default=default_serializer
            )
...
def generate_html_report(
    analysis: Dict, symbol: str, filename: str = "analysis_report.html"
) -> str:
    """生成互動HTML報告"""

    ranked = sorted(
        [(p, a["score"]) for p, a in analysis.items()], key=lambda x: x[1], reverse=True
    )
